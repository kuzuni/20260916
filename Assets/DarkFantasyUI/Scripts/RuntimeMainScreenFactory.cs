...
namespace Moonlit.UI
{
    public sealed partial class RuntimeMainScreenFactory
    {
        ...
        public MainScreen Create(Transform root)
        {
            ...
            var canvas=new GameObject("Runtime Canvas",typeof(RectTransform),typeof(Canvas),typeof(CanvasScaler),typeof(GraphicRaycaster)).GetComponent<Canvas>();
            ...
            var floor=Ui.Image("Full-bleed stone",canvas.transform,0,0,1080,1920,panels[4],new Color(.4f,.4f,.4f)); floor.type=Image.Type.Tiled; Ui.Stretch(floor.rectTransform);
            var world=Ui.Rect("Elastic battle viewport",canvas.transform,0,0,1080,1000); world.gameObject.AddComponent<RectMask2D>();
            var worldArt=Ui.Image("Moonlit scenery",world,0,0,1080,1080,assets.worldBackground);
            var safeFrame=Ui.Rect("Device Safe Area",canvas.transform,0,0,1080,1920); Ui.Stretch(safeFrame);
            ...
            BuildForgeAndChat(main,bottom); BuildNavigation(main,bottom);
