...
복구 스크립트는 `tools/repair_chihuahua_psb.py`, 이번 파일 검사 결과는 `Artifacts/PSBRepair-20260918/verification.json`, 교체 전 백업은 같은 폴더의 `before.zip`에 있습니다. 27개 파일을 다시 열어 7개 레이어와 픽셀 일치를 검사하고, 별도로 구현한 RLE 디코더로 모든 레이어 및 합성 데이터와 파일 경계를 검증했습니다. Unity 에디터 실행·클라우드 임포트·Photoshop 앱에서의 직접 열기 검증은 수행하지 않았습니다. 생성 당시 프롬프트·이미지 해시 기록은 기존 `tools/chihuahua_art/generation_provenance.json` 경로를 참조합니다.
